    ...
    statusFrame(1)
{
    ...
    GdkPixbuf *logo = gtk_icon_theme_load_icon(iconTheme, "skylitedcpp", 128, GTK_ICON_LOOKUP_FORCE_SVG, NULL);
    ...
    gtk_about_dialog_set_website(GTK_ABOUT_DIALOG(getWidget("aboutDialog")), "http://skylitedc.googlecode.com/");
    gtk_window_set_transient_for(GTK_WINDOW(getWidget("aboutDialog")), window);

    // Set all windows to the default icon
    gtk_window_set_default_icon_name("skylitedcpp");
    ...
    g_signal_connect(getWidget("uploadQueueItem"), "activate", G_CALLBACK(onUploadQueueClicked_gui), (gpointer)this);
    ...
    g_signal_connect(getWidget("CloseTabPMOfflineItem"), "activate", G_CALLBACK(onCloseAlloffPM_gui), (gpointer)this);g_signal_connect(getWidget("ReconnectAllHubsItem"), "activate", G_CALLBACK(onReconectAllHub_gui), (gpointer)this);

    // Help menu
    g_object_set_data_full(G_OBJECT(getWidget("homeMenuItem")), "link",
        g_strdup("http://code.google.com/p/skylitedc/"), g_free);
    ...
    g_object_set_data_full(G_OBJECT(getWidget("sourceMenuItem")), "link",
        g_strdup("http://github.com/skylitedcpp/skylitedcpp/"), g_free);
    ...
    g_object_set_data_full(G_OBJECT(getWidget("issueMenuItem")), "link",
        g_strdup("http://code.google.com/p/skylitedc/issues/list"), g_free);
    ...
    g_object_set_data_full(G_OBJECT(getWidget("wikiItem")), "link",
        g_strdup("http://code.google.com/p/skylitedc/w/list"), g_free);
    ...
    g_object_set_data_full(G_OBJECT(getWidget("changeLogItem")), "link",
        g_strdup(_("http://github.com/skylitedcpp/skylitedcpp/raw/master/ChangeLog.txt")), g_free);
...
void MainWindow::createStatusIcon_gui()
{
    ...
    statusIcon = gtk_status_icon_new_from_icon_name("skylitedcpp");
...
void MainWindow::removeTimerSource_gui()
{
    if (timer > 0)
    {
        g_source_remove(timer);
        timer = 0;
        gtk_status_icon_set_from_icon_name(statusIcon, "skylitedcpp");
    }
}
...
gboolean MainWindow::animationStatusIcon_gui(gpointer data)
{
    ...
    if (mw->isActive_gui())
    {
        gtk_status_icon_set_from_icon_name(mw->statusIcon, "skylitedcpp");
        mw->timer = 0;

        return FALSE;
    }

    gtk_status_icon_set_from_icon_name(mw->statusIcon, (mw->statusFrame *= -1) > 0 ? "skylitedcpp" : "icon_msg");
...
void MainWindow::showPortsError(const string& port) {
    string msg = str(dcpp_fmt(dgettext("skylitedcpp-gtk", "Unable to open %1% port. Searching or file transfers will not work correctly until you change settings or turn off any application that might be using that port.")) % port);
    typedef Func2<MainWindow, string, string> F2;
