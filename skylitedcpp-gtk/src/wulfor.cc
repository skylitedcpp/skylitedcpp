...
#define GUI_PACKAGE "skylitedcpp-gtk"
...
void printHelp()
{
    printf(_(
            "Using:\n"
            "  skylitedcpp-gtk <magnet link> <dchub://link> <adc(s)://link>\n"
            "  skylitedcpp-gtk <Key>\n"
...
int main(int argc, char *argv[])
{
    ...
    dcdebug(connection ? "skylitedcpp-gtk: connection yes...\n" : "skylitedcpp-gtk: connection no...\n");
    ...
    if (WulforUtil::profileIsLocked())
    {
        ...
        {
            dcdebug("skylitedcpp-gtk: is client...\n");

            if (argc > 1)
            {
                dcdebug("skylitedcpp-gtk: send %s\n", argv[1]);
    ...
    if (bacon_message_connection_get_is_server(connection))
    {
        dcdebug("skylitedcpp-gtk: is server...\n");
    ...
    gdk_threads_init();
    gtk_init(&argc, &argv);
    g_set_application_name("SkyliteDC++ Gtk");
